...
import os
from factorlib.configs.syspath import (BASE_PATH, DATA_PATH, UNIVERSE_PATH, FACTOR_VALUES_PATH,
...
def main(factor_name,configFile):
    start_time = time.time()
    with open(configFile, 'r',encoding='utf-8') as f:
        config = yaml.safe_load(f)
    factor_params = next((f for f in config['factors'] if f['name'] == factor_name), None)
